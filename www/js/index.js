...
var db = window.openDatabase('ThongCW', '1.0', 'ThongCW', 20000);
...
$(document).on('pagebeforeshow', '#page-create', function () {
    ...
    addSelectOption('#page-create #frm-register #Furniture_types', Furniture, 'Furniture');
    ...
    db.transaction(function (tx) {
        ...
        function transactionSuccess(tx, result) {
            ...
            for (let item of result.rows) {
                optionList += `<option value="${item.Id}" ${item.Name === selectedId ? 'selected' : ''}> ${item.Name}</option> `;
            }
...
const checkCommonValidation = (selectInput, errorSelect = '', name, min = -1, max = -1) => {
...
const checkPriceValidation = (selectInput, errorSelect = '', name, min = -1, max = -1) => {
    let value = document.querySelector(selectInput).value;
...
$(document).on('submit', '#page-create #frm-register', function (e) {
    e.preventDefault();

    checkCommonValidation('#page-create #frm-register #Property_name', '#page-create #frm-register #error_name', 'Property Name', 12, 120);
    checkCommonValidation('#page-create #frm-register #Property_address', '#page-create #frm-register #error_address', 'Property Address', 6, 150);
    checkCharacterValidation('#page-create #frm-register #Reporter_name', '#page-create #frm-register #error_reporter', 'Reporter Name', 3, 120);
    checkPriceValidation('#page-create #frm-register #Rent_Price', '#page-create #frm-register #error_price', 'Monthly Price', 500000, 500000000);
    checkSelectValidation('#page-create #frm-register #Property_type', '#error_type', 'Property Type');
    checkSelectValidation('#page-create #frm-register #Bedroom', '#error_bedroom', 'Property Bedroom');
    checkSelectValidation('#page-create #frm-register #Furniture_types', '#error_furniture', 'Property Furniture');
    checkSelectValidation('#page-create #frm-register #city', '#error_city', 'City');
    checkSelectValidation('#page-create #frm-register #district', '#error_district', 'District');
    checkSelectValidation('#page-create #frm-register #ward', '#error_ward', 'Ward');

    errorName = document.querySelector('#page-create #frm-register #error_name').innerText;
    errorPrice = document.querySelector('#page-create #frm-register #error_price').textContent;
    errorAddress = document.querySelector('#page-create #frm-register #error_address').textContent;
    errorReporter = document.querySelector('#page-create #frm-register #error_reporter').textContent;
    errorType = document.querySelector('#page-create #frm-register #error_type').textContent;
    errorBedroom = document.querySelector('#page-create #frm-register #error_bedroom').textContent;
    errorFurniture = document.querySelector('#page-create #frm-register #error_furniture').textContent;
    errorCity = document.querySelector('#page-create #frm-register #error_city').textContent;
    errorDistrict = document.querySelector('#page-create #frm-register #error_district').textContent;
    errorWard = document.querySelector('#page-create #frm-register #error_ward').textContent;
...
$(document).on('vclick', '#page-create #popup-register-confirm #btn-confirm', registerPost);
    ...
    db.transaction(function (tx) {
        ...
        function transactionSuccess(tx, result) {
            if (result.rows[0] == null) {
                $('#page-create #popup-register-confirm #Property_name').text(propertyname);
                $('#page-create #popup-register-confirm #Property_address').text(propertyaddress);
                ...
                $('#page-create #popup-register-confirm #Property_type').text(propertytype);
                $('#page-create #popup-register-confirm #Bedroom').text(bedroom);
                $('#page-create #popup-register-confirm #Rent_Price').text(rentprice);
                $('#page-create #popup-register-confirm #Furniture_types').text(furnituretype);
                $('#page-create #popup-register-confirm #Note').text(note);
                $('#page-create #popup-register-confirm #Reporter_name').text(reportername);
...
function registerPost(e) {
    ...
    db.transaction(function (tx) {
        var query = 'INSERT INTO Post (Property_name, Property_address, City, District, Ward, Property_type, Bedroom, DateTime, Rent_Price, Furniture_type, Reporter) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
        tx.executeSql(query, [propertyname, propertyaddress, city, district, ward, propertytype, bedroom, dateTime, rentprice, furnituretype, reportername], transactionSuccess, transactionError);
    ...
    db.transaction(function (tx) {
        ...
        function transactionSuccess(tx, result) {
            ...
            for (let post of result.rows) {
                listPost += `<li> <a data-details='{"Id" : ${post.Id}}' style="padding: 0.7em 0.5em; background-color: #2A2438; border: none !important;">
                <div class="card">
                    <div><strong>${post.Type}</strong> - ${post.Name}</div>
                    <p style="margin-left: 10px;" class="flex"><i class="fas fa-map-marked-alt" style="font-size: 20px; margin-right: 20px;"></i>${post.District} - ${post.City}</p>
                    <div class="flex-between" style="margin: 30px 20px 10px 10px">
                        <div class="flex-column">
                            <i class="fas fa-bed"></i>
                            <p style="font-size: 14px; font-weight: 300; text-shadow: none !important;">${post.Bedroom}</p>
                        </div>
...
function showDetail() {
    ...
    db.transaction(function (tx) {
        ...
        function transactionSuccess(tx, result) {
            ...
            $('#page-detail #date').text(moment(dateTime).format('dddd, MMMM Do YYYY, h:mm:ss A'));
...
$(document).on('change', '#page-detail #frm-update #city', function () {
    importAddressUpdate('#page-detail #frm-update', 'District', -1, 'City', this.value);
    importAddressUpdate('#page-detail #frm-update', 'Ward', -1, 'District', this.value);
});
...
function importAddressUpdate(from, name, selectedId = -1, parentName = '', selectedParentId = -1) {
    var lowerName = name.toLowerCase();
    // var selectedName = $('#page-create #frm-register #city option:selected').text();
    var id, condition = ''
    if (parentName) {
...
$(document).on("vclick", "#page-detail #btn-update-popup", function () {
    ...
    db.transaction(function (tx) {
        ...
        function transactionSuccess(tx, result) {
            if (result.rows[0] != null) {
                $(`#page-detail #frm-update #update-name`).val(result.rows[0].Property_name);
                $(`#page-detail #frm-update #update-rent`).val(result.rows[0].Rent_Price);
                $(`#page-detail #frm-update #update-address`).val(result.rows[0].Property_address);
                $(`#page-detail #frm-update #update-reporter`).val(result.rows[0].Reporter);

                addSelectOption('#page-detail #frm-update #update-type', Type, 'Type', result.rows[0].Property_type);
                addSelectOption('#page-detail #frm-update #update-furniture', Furniture, 'Furniture', result.rows[0].Furniture_type);
                addSelectOption('#page-detail #frm-update #update-bedroom', Bedroom, 'Bedroom', result.rows[0].Bedroom);
                ...
                importAddressUpdate('#page-detail #frm-update', 'City', result.rows[0].City);
                importAddressUpdate('#page-detail #frm-update', 'District', result.rows[0].District, 'City', result.rows[0].City);
                importAddressUpdate('#page-detail #frm-update', 'Ward', result.rows[0].Ward, 'District', result.rows[0].District);
            }
        }
    });
});

$(document).on('submit', '#page-detail #frm-update', function (e) {
    e.preventDefault();

    checkCommonValidation('#page-detail #frm-update #update-name', '#page-detail #frm-update #error_name', 'Property Name', 12, 120);
    checkCommonValidation('#page-detail #frm-update #update-address', '#page-detail #frm-update #error_address', 'Property Address', 6, 150);
    checkCharacterValidation('#page-detail #frm-update #update-reporter', '#page-detail #frm-update #error_reporter', 'Reporter Name', 3, 120);
    checkPriceValidation('#page-detail #frm-update #update-rent', '#page-detail #frm-update #error_price', 'Monthly Price', 500000, 500000000);
    checkSelectValidation('#page-detail #frm-update #update-type', '#page-detail #frm-update #error_type', 'Property Type');
    checkSelectValidation('#page-detail #frm-update #update-bedroom', '#page-detail #frm-update #error_bedroom', 'Property Bedroom');
    checkSelectValidation('#page-detail #frm-update #update-furniture', '#page-detail #frm-update #error_furniture', 'Property Furniture');
    checkSelectValidation('#page-detail #frm-update #city', '#page-detail #frm-update #error_city', 'City');
    checkSelectValidation('#page-detail #frm-update #district', '#page-detail #frm-update #error_district', 'District');
    checkSelectValidation('#page-detail #frm-update #ward', '#page-detail #frm-update #error_ward', 'Ward');


    errorName = document.querySelector('#page-detail #frm-update #error_name').innerText;
    errorPrice = document.querySelector('#page-detail #frm-update #error_price').textContent;
    errorAddress = document.querySelector('#page-detail #frm-update #error_address').textContent;
    errorReporter = document.querySelector('#page-detail #frm-update #error_reporter').textContent;
    errorType = document.querySelector('#page-detail #frm-update #error_type').textContent;
    errorBedroom = document.querySelector('#page-detail #frm-update #error_bedroom').textContent;
    errorFurniture = document.querySelector('#page-detail #frm-update #error_furniture').textContent;
    errorCity = document.querySelector('#page-detail #frm-update #error_city').textContent;
    errorDistrict = document.querySelector('#page-detail #frm-update #error_district').textContent;
    errorWard = document.querySelector('#page-detail #frm-update #error_ward').textContent;
...
function updatePost() {
    ...
    db.transaction(function (tx) {
        ...
        var query = "UPDATE Post SET Property_name = ?, Property_address = ?, City = ? , District = ?, Ward = ? , Property_type = ?, Bedroom = ?, Rent_Price = ?, Furniture_type = ?, Reporter = ?, DateTime = ? WHERE id = ?";
        tx.executeSql(query, [propertyname, propertyaddress, city, district, ward, propertytype, bedroom, rentprice, furnituretype, reportername, dateTime, id], transactionSuccess, transactionError);
...
function confirmDeletePost() {
    ...
    // var text = $(document).on('change', '#page-detail #frm-delete #txt-delete');
    ...
    db.transaction(function (tx) {
        ...
        function transactionSuccess(tx, result) {
            ...
            $(document).on('keyup input', '#page-detail #frm-delete #txt-delete', function () {
                var text = this.value;
            ...
            // $('#page-detail #frm-delete #txt-delete').keyup(function () {
            //     var text = this.value;
            //     console.log('textKeyPress', text);
...
function addNote(e) {
    ...
    if (note) {
        db.transaction(function (tx) {
            var query = 'INSERT INTO Note (Message, DateTime, PostId) VALUES (?, ?, ?)';
            tx.executeSql(query, [note, dateTime, postId], transactionSuccess, transactionError);
...
$(document).on('vclick', '#page-detail #list-note .note #btn-delete-note', deleteNote)

function deleteNote() {
    var postId = localStorage.getItem('currentPostId');

    var noteId = parseInt(document.querySelector('#page-detail #list-note .note').id);
    ...
    db.transaction(function (tx) {
        ...
        function transactionSuccess(tx, result) {
            ...
            for (let note of result.rows) {
                ...
                listNote += `<div style="position: relative;" id=${note.Id} class="note">
                                <button id='btn-delete-note'><i class="fas fa-trash-alt"></i></button>
                                <div class='message-text'>${note.Message}</div>
                                <div class='message-hour'>${moment(note.DateTime).format('dddd, h:mm:ss A')}</div>
                            </div>`
            }
...
$(document).on('pagebeforeshow', '#page-search', function () {
    ...
    addSelectOption('#page-search #frm-search #Furniture_types', Furniture, 'Furniture');
...
$(document).on('submit', '#page-search #frm-search', (e) => {
    e.preventDefault();
    ...
    db.transaction(function (tx) {
        ...
        function transactionSuccess(tx, result) {
            ...
            if (result.rows.length === 0) {
                document.querySelector('#page-search #search-message').innerHTML = 'Post not Found.'
            }
            else {
            ...
            for (let post of result.rows) {
                listPost += `<li> <a data-details='{"Id" : ${post.Id}}' style="padding: 0.7em 0.5em; background-color: transparent !important; border-color: transparent !important">
                <div class="card">
                    <div class="title"><strong>${post.Type}</strong> - ${post.Name}</div>
                    <p style="margin-left: 10px;" class="flex"><i class="fas fa-map-marked-alt" style="font-size: 20px; margin-right: 20px;"></i>${post.City}</p>
                    <div class="flex-between" style="margin: 30px 20px 10px 10px">
                        <div class="flex-column">
                            <i class="fas fa-bed"></i>
                            <p style="font-size: 14px; font-weight: 300;">${post.Bedroom}</p>
                        </div>
                        <div class="flex-column" style="max-width:50%; width: 100%">
                            <i class="fas fa-hand-holding-usd"></i>
            ...
            // Add list to UI.
            $('#page-search #list-post').empty().append(listPost).listview('refresh').trigger('create');
            log(`Show list of posts successfully.`);
        }
    });
}
